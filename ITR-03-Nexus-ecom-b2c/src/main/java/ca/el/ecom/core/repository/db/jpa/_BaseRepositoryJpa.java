...
import ca.el.ecom.core.model.entity._SortableEntity;
...
public abstract class _BaseRepositoryJpa<T extends _BaseEntity<ID>, ID extends Serializable> implements _BaseRepository<T, ID> {
   ...
   protected List<T> findList(CriteriaQuery<T> criteriaQuery, Integer first, Integer count, List<Filter> filters, List<Sort> sorts) {
      ...
      if (orderList.isEmpty()) {
         if (_SortableEntity.class.isAssignableFrom(entityClass)) {
            orderList.add(criteriaBuilder.asc(getPath(root, _SortableEntity.ORDER_PROPERTY_NAME)));
         }
         else {
            orderList.add(criteriaBuilder.desc(getPath(root, _SortableEntity.CREATE_DATE_PROPERTY_NAME)));
         }
      }
